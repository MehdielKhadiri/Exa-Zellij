...
KOKKOS_INLINE_FUNCTION
double svmPredict(const double* x, const double* support_vector, int features, double gamma) {
    double sum = 0.0;
    for (int i = 0; i < features; ++i) {
        double diff = x[i] - support_vector[i];
        sum += diff * diff;
    }
    return exp(-gamma * sum);
}

// Compute the SVM-like objective function (error to be minimized)
template<class ViewType>
KOKKOS_INLINE_FUNCTION
double objectiveFunction_impl(const ViewType& point) {
    int dimension = point.extent(0);
    
    // First three parameters are C, gamma, and epsilon
    double C = point(0);
    double gamma = point(1);
    double epsilon = point(2);
    
    int features = (dimension - 3) / 2;  // Half for "training", half for "testing"
    
    // Simple "training" process: just use the first half as support vectors
    const double* support_vectors = &point(3);
    
    // "Testing" process: compute error on the second half
    double error = 0.0;
    for (int i = 0; i < features; ++i) {
        double prediction = svmPredict(&point(3 + features + i), support_vectors, features, gamma);
        double true_value = point(3 + 2*features + i);  // Assuming last 'features' elements are true values
        double loss = Kokkos::fmax(0.0, Kokkos::fabs(prediction - true_value) - epsilon);
        error += C * loss;
    }
    
    return error / features;  // Average error
}

// Overload for Kokkos::View
template<class ViewType>
KOKKOS_INLINE_FUNCTION
double objectiveFunction(const ViewType& point) {
    return objectiveFunction_impl(point);
}

// Overload for std::vector
double objectiveFunction(const std::vector<double>& point) {
    Kokkos::View<double*, Kokkos::HostSpace> point_view("point_view", point.size());
    for (size_t i = 0; i < point.size(); ++i) {
        point_view(i) = point[i];
    }
    return objectiveFunction_impl(point_view);
}

// Overload for raw pointer
double objectiveFunction(const double* point, int dimension) {
    Kokkos::View<const double*, Kokkos::HostSpace> point_view(point, dimension);
    return objectiveFunction_impl(point_view);
}
...
int main(int argc, char* argv[]) {
    ...
    {   auto start = std::chrono::high_resolution_clock::now();
        ...
        // Synchronize minObjective across all MPI processes
        MPI_Allreduce(MPI_IN_PLACE, &minObjective, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
